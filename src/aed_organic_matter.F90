...
!BEGIN
   ...
   IF ( aed_write_nml_mode ) THEN
      ...
      WRITE(namlst,'(A)') "&aed_organic_matter"
      CALL aed_nml_wr(namlst, 'om_min',                        om_min,                        'Minimum OM-state concentration; enforces negative-number clipping')
      CALL aed_nml_wr(namlst, 'om_max',                        om_max,                        'Maximum OM-state concentration; enforces high-number clipping')
      CALL aed_nml_wr(namlst, 'poc_initial',                   poc_initial,                   'Initial particulate organic carbon (POC) concentration (mmol C/m3)')
      CALL aed_nml_wr(namlst, 'pon_initial',                   pon_initial,                   'Initial particulate organic nitrogen (PON) concentration (mmol N/m3)')
      CALL aed_nml_wr(namlst, 'pop_initial',                   pop_initial,                   'Initial particulate organic phosphorus (POP) concentration (mmol P/m3)')
      CALL aed_nml_wr(namlst, 'doc_initial',                   doc_initial,                   'Initial dissolved organic carbon (DOC) concentration (mmol C/m3)')
      CALL aed_nml_wr(namlst, 'don_initial',                   don_initial,                   'Initial dissolved organic nitrogen (DON) concentration (mmol N/m3)')
      CALL aed_nml_wr(namlst, 'dop_initial',                   dop_initial,                   'Initial dissolved organic phosphorus (DOP) concentration (mmol P/m3)')
      CALL aed_nml_wr(namlst, 'docr_initial',                  docr_initial,                  'Initial refractory DOC concentration (mmol C/m3)')
      CALL aed_nml_wr(namlst, 'donr_initial',                  donr_initial,                  'Initial refractory DON concentration (mmol N/m3)')
      CALL aed_nml_wr(namlst, 'dopr_initial',                  dopr_initial,                  'Initial refractory DOP concentration (mmol P/m3)')
      CALL aed_nml_wr(namlst, 'cpom_initial',                  cpom_initial,                  'Initial coarse particulate OM (CPOM) concentration (mmol C/m3)')
      CALL aed_nml_wi(namlst, 'simDenitrification',            simDenitrification,            'Denitrification pathway: 0=off')
      CALL aed_nml_wi(namlst, 'simFeReduction',                simFeReduction,                'Iron-reduction pathway: 0=off')
      CALL aed_nml_wi(namlst, 'simSO4Reduction',                simSO4Reduction,               'Sulfate-reduction pathway: 0=off')
      CALL aed_nml_wi(namlst, 'simMethanogenesis',              simMethanogenesis,             'Methanogenesis pathway: 0=off')
      CALL aed_nml_wr(namlst, 'Rpom_hydrol',                    Rpom_hydrol,                   'POM hydrolysis rate, combined pool (/day)')
      CALL aed_nml_wr(namlst, 'Rpoc_hydrol',                    Rpoc_hydrol,                   'POC hydrolysis rate (/day)')
      CALL aed_nml_wr(namlst, 'Rpon_hydrol',                    Rpon_hydrol,                   'PON hydrolysis rate (/day)')
      CALL aed_nml_wr(namlst, 'Rpop_hydrol',                    Rpop_hydrol,                   'POP hydrolysis rate (/day)')
      CALL aed_nml_wr(namlst, 'Rdom_minerl',                    Rdom_minerl,                   'DOM mineralisation rate, combined pool (/day)')
      CALL aed_nml_wr(namlst, 'Rdoc_minerl',                    Rdoc_minerl,                   'DOC mineralisation rate (/day)')
      CALL aed_nml_wr(namlst, 'Rdon_minerl',                    Rdon_minerl,                   'DON mineralisation rate (/day)')
      CALL aed_nml_wr(namlst, 'Rdop_minerl',                    Rdop_minerl,                   'DOP mineralisation rate (/day)')
      CALL aed_nml_wr(namlst, 'theta_hydrol',                   theta_hydrol,                  'Arrhenius temperature multiplier for hydrolysis')
      CALL aed_nml_wr(namlst, 'theta_minerl',                   theta_minerl,                  'Arrhenius temperature multiplier for mineralisation')
      CALL aed_nml_wr(namlst, 'Kpom_hydrol',                    Kpom_hydrol,                   'Half-saturation oxygen concentration for POM hydrolysis (mmol O2/m3)')
      CALL aed_nml_wr(namlst, 'Kdom_minerl',                    Kdom_minerl,                   'Half-saturation oxygen concentration for DOM mineralisation (mmol O2/m3)')
      CALL aed_nml_wr(namlst, 'f_an',                           f_an,                          'Anaerobic-condition scaling factor applied to hydrolysis/mineralisation')
      CALL aed_nml_ws(namlst, 'dom_miner_oxy_reactant_var',     dom_miner_oxy_reactant_var,    'Link to aed_oxygen; oxic mineralisation electron acceptor')
      CALL aed_nml_ws(namlst, 'dom_miner_nit_reactant_var',     dom_miner_nit_reactant_var,    'Link to aed_nitrogen nitrate; denitrification electron acceptor')
      CALL aed_nml_ws(namlst, 'dom_miner_no2_reactant_var',     dom_miner_no2_reactant_var,    'Link to aed_nitrogen nitrite; denitrification electron acceptor')
      CALL aed_nml_ws(namlst, 'dom_miner_n2o_reactant_var',     dom_miner_n2o_reactant_var,    'Link to aed_nitrogen N2O; denitrification electron acceptor')
      CALL aed_nml_ws(namlst, 'dom_miner_fe3_reactant_var',     dom_miner_fe3_reactant_var,    'Link to aed_geochemistry Fe(III); iron-reduction electron acceptor')
      CALL aed_nml_ws(namlst, 'dom_miner_so4_reactant_var',     dom_miner_so4_reactant_var,    'Link to aed_geochemistry sulfate; sulfate-reduction electron acceptor')
      CALL aed_nml_ws(namlst, 'dom_miner_ch4_reactant_var',     dom_miner_ch4_reactant_var,    'Link to aed_methane; methanogenesis product')
      CALL aed_nml_ws(namlst, 'doc_miner_product_variable',     doc_miner_product_variable,    'Link to the state variable receiving mineralised DOC (e.g. DIC)')
      CALL aed_nml_ws(namlst, 'don_miner_product_variable',     don_miner_product_variable,    'Link to the state variable receiving mineralised DON (e.g. ammonium)')
      CALL aed_nml_ws(namlst, 'dop_miner_product_variable',     dop_miner_product_variable,    'Link to the state variable receiving mineralised DOP (e.g. FRP)')
      CALL aed_nml_wl(namlst, 'simRPools',                      simRPools,                     'Enable the refractory OM pools (docr/donr/dopr, cpom)')
      CALL aed_nml_wr(namlst, 'Rdomr_minerl',                   Rdomr_minerl,                  'Refractory DOM mineralisation rate (/day)')
      CALL aed_nml_wr(namlst, 'Rcpom_bdown',                    Rcpom_bdown,                   'CPOM breakdown rate to the refractory DOM pool (/day)')
      CALL aed_nml_wr(namlst, 'X_cpom_n',                       X_cpom_n,                      'N:C stoichiometric ratio of CPOM')
      CALL aed_nml_wr(namlst, 'X_cpom_p',                       X_cpom_p,                      'P:C stoichiometric ratio of CPOM')
      CALL aed_nml_wr(namlst, 'Kin_denitrat',                   Kin_denitrat,                  'Half-saturation oxygen concentration inhibiting nitrate denitrification (mmol O2/m3)')
      CALL aed_nml_wr(namlst, 'Kin_denitrit',                   Kin_denitrit,                  'Half-saturation oxygen concentration inhibiting nitrite denitrification (mmol O2/m3)')
      CALL aed_nml_wr(namlst, 'Kin_denitrous',                  Kin_denitrous,                 'Half-saturation oxygen concentration inhibiting N2O reduction (mmol O2/m3)')
      CALL aed_nml_wr(namlst, 'K_nit',                          K_nit,                         'Half-saturation nitrate concentration for denitrification (mmol N/m3)')
      CALL aed_nml_wr(namlst, 'Klim_denitrous',                 Klim_denitrous,                'Half-saturation N2O concentration limiting N2O reduction (mmol N/m3)')
      CALL aed_nml_wr(namlst, 'Klim_denitrit',                  Klim_denitrit,                 'Half-saturation nitrite concentration limiting denitrification (mmol N/m3)')
      CALL aed_nml_wr(namlst, 'Kpart_denitrit',                 Kpart_denitrit,                'Partitioning constant between nitrite and N2O denitrification steps')
      CALL aed_nml_wr(namlst, 'KeDOM',                          KeDOM,                         'Light extinction coefficient per unit DOM (/m per mmol C/m3)')
      CALL aed_nml_wr(namlst, 'KePOM',                          KePOM,                         'Light extinction coefficient per unit POM (/m per mmol C/m3)')
      CALL aed_nml_wr(namlst, 'KeDOMR',                         KeDOMR,                        'Light extinction coefficient per unit refractory DOM (/m per mmol C/m3)')
      CALL aed_nml_wr(namlst, 'KeCPOM',                         KeCPOM,                        'Light extinction coefficient per unit CPOM (/m per mmol C/m3)')
      CALL aed_nml_wl(namlst, 'simPhotolysis',                  simPhotolysis,                 'Enable photolysis of refractory DOM')
      CALL aed_nml_wr(namlst, 'photo_fmin',                     photo_fmin,                    'Minimum fraction of refractory DOM resistant to photolysis')
      CALL aed_nml_wr(namlst, 'photo_c',                        photo_c,                       'Photolysis rate coefficient')
      CALL aed_nml_wi(namlst, 'settling',                       settling,                      'Settling model for POM/CPOM: 0=off, other values module-specific')
      CALL aed_nml_wr(namlst, 'w_pom',                          w_pom,                         'POM settling velocity (m/day)')
      CALL aed_nml_wr(namlst, 'd_pom',                          d_pom,                         'POM particle diameter (m)')
      CALL aed_nml_wr(namlst, 'rho_pom',                        rho_pom,                       'POM particle density (kg/m3)')
      CALL aed_nml_wr(namlst, 'w_cpom',                         w_cpom,                        'CPOM settling velocity (m/day)')
      CALL aed_nml_wr(namlst, 'd_cpom',                         d_cpom,                        'CPOM particle diameter (m)')
      CALL aed_nml_wr(namlst, 'rho_cpom',                       rho_cpom,                      'CPOM particle density (kg/m3)')
      CALL aed_nml_ws(namlst, 'Psed_poc_variable',              Psed_poc_variable,             'Link to the POC particle-settling flux diagnostic')
      CALL aed_nml_ws(namlst, 'Psed_pon_variable',              Psed_pon_variable,             'Link to the PON particle-settling flux diagnostic')
      CALL aed_nml_ws(namlst, 'Psed_pop_variable',              Psed_pop_variable,             'Link to the POP particle-settling flux diagnostic')
      CALL aed_nml_ws(namlst, 'Psed_cpom_variable',             Psed_cpom_variable,            'Link to the CPOM particle-settling flux diagnostic')
      CALL aed_nml_ws(namlst, 'pom_particle_link',              pom_particle_link,             'Link to the particle-tracking variable representing POM')
      CALL aed_nml_ws(namlst, 'cpom_particle_link',             cpom_particle_link,            'Link to the particle-tracking variable representing CPOM')
      ...
      CALL aed_nml_wr(namlst, 'sedimentOMfrac',                 sedimentOMfrac,                'Organic matter fraction of the sediment bed (w/w)')
      CALL aed_nml_wr(namlst, 'sedimentBulkDens',                sedimentBulkDens,              'Sediment bulk density (kg/m3)')
      CALL aed_nml_wr(namlst, 'sedimentDepth',                   sedimentDepth,                 'Active sediment layer depth (m)')
      CALL aed_nml_wr(namlst, 'Xsc',                             Xsc,                           'Sediment organic carbon stoichiometric fraction')
      CALL aed_nml_wr(namlst, 'Xsn',                             Xsn,                           'Sediment organic nitrogen stoichiometric fraction')
      CALL aed_nml_wr(namlst, 'Xsp',                             Xsp,                           'Sediment organic phosphorus stoichiometric fraction')
      CALL aed_nml_wr(namlst, 'Fsed_doc',                        Fsed_doc,                      'Sediment DOC flux (mmol C/m2/day)')
      CALL aed_nml_wr(namlst, 'Fsed_don',                        Fsed_don,                      'Sediment DON flux (mmol N/m2/day)')
      CALL aed_nml_wr(namlst, 'Fsed_dop',                        Fsed_dop,                      'Sediment DOP flux (mmol P/m2/day)')
      CALL aed_nml_wr(namlst, 'Ksed_dom',                        Ksed_dom,                      'Half-saturation oxygen concentration for sediment DOM flux (mmol O2/m3)')
      CALL aed_nml_wr(namlst, 'theta_sed_dom',                   theta_sed_dom,                 'Arrhenius temperature multiplier for sediment DOM flux')
      CALL aed_nml_ws(namlst, 'Fsed_poc_variable',               Fsed_poc_variable,             'Link to aed_sedflux for a spatially/temporally varying Fsed_poc')
      CALL aed_nml_ws(namlst, 'Fsed_doc_variable',               Fsed_doc_variable,             'Link to aed_sedflux for a spatially/temporally varying Fsed_doc')
      CALL aed_nml_ws(namlst, 'Fsed_pop_variable',               Fsed_pop_variable,             'Link to aed_sedflux for a spatially/temporally varying Fsed_pop')
      CALL aed_nml_ws(namlst, 'Fsed_dop_variable',               Fsed_dop_variable,             'Link to aed_sedflux for a spatially/temporally varying Fsed_dop')
      CALL aed_nml_ws(namlst, 'Fsed_pon_variable',               Fsed_pon_variable,             'Link to aed_sedflux for a spatially/temporally varying Fsed_pon')
      CALL aed_nml_ws(namlst, 'Fsed_don_variable',               Fsed_don_variable,             'Link to aed_sedflux for a spatially/temporally varying Fsed_don')
      CALL aed_nml_wi(namlst, 'simFlocculation',                 simFlocculation,               'DOM flocculation pathway: 0=off')
      CALL aed_nml_wr(namlst, 'Rdom_floc',                       Rdom_floc,                     'DOM flocculation rate (/day)')
      CALL aed_nml_wr(namlst, 'Kdom_floc',                       Kdom_floc,                     'Half-saturation DOM concentration for flocculation (mmol C/m3)')
      CALL aed_nml_wr(namlst, 'dom_floc0',                       dom_floc0,                     'Reference DOM concentration for the flocculation response (mmol C/m3)')
      CALL aed_nml_wr(namlst, 'dom_flocS',                       dom_flocS,                     'Flocculation response steepness/shape parameter')
